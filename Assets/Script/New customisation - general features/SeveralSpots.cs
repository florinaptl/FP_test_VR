...
public class SeveralSpots : MonoBehaviour
{
...

    {
        ...
        {
            //use timer as speed
            if (myTime == nrOfCurrentSequence * sequenceDuration + 3f)
            {
                ok = 1;
                timer = 0.0288f;
            }

            if (myTime > nrOfCurrentSequence * sequenceDuration + 3f && timer < 15 / 34.67)
            {
    ...
    public void ChangeNrOfSpots(ref List<Spot> spotsList, int _nrOfSpots)
    {
        if (_nrOfSpots != spotsList.Count)
        {
            ...
            {
                MakeSpots(_nrOfSpots - spotsList.Count, -1f, 5.5f, centerLine - facadeHeight / 2 * compactness_facade_percentage, centerLine + facadeHeight / 2 * compactness_facade_percentage);
            }
